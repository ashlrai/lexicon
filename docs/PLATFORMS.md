...
|---|---|---|---|---|
| CLI: all twenty-five `lexicon` commands | **Tested in CI** | **Tested in CI** | **Tested in CI** | The 984 TypeScript tests run on every matrix job, followed by a smoke test that drives the built binary rather than the source. Only the jsdom extension file is conditional, and only on Node 20. |
...
| In-place correction in any app | **Tested in CI** | **Works, untested on real hardware** | **Not available** | macOS: 228 Swift tests plus a release build in CI, and the live Accessibility path verified by hand in TextEdit, the Codex desktop app and Safari. Windows: 214 headless C# tests run in CI on both Linux and `windows-latest` and the binary cross-builds from macOS, but every line that touches UI Automation is unrun. Linux: no tray app exists. See [System-wide in-place correction](#system-wide-in-place-correction). |
