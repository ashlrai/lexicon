...
|---|---|---|---|---|
| CLI: all twenty-five `lexicon` commands | **Tested in CI** | **Tested in CI** | **Tested in CI** | The 926 TypeScript tests run on every matrix job, followed by a smoke test that drives the built binary rather than the source. Only the jsdom extension file is conditional, and only on Node 20. |
