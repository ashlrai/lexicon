...
**A keystroke write that half-lands** used to be the one thing the Windows app had a recovery for and this one did not, and the one remaining way LexiconBar could leave text it could not put back. Key events were posted in chunks of twenty UTF-16 units with no accounting: a chunk whose `CGEvent` could not be built was skipped in silence while the chunks after it went out anyway, so a failure in the middle left a hole in the middle. The engine then re-read the field, found it held neither the correction nor the text the plan was made from, reported "typing changed the field to something else; left alone" and recorded nothing. The field held half a correction, the user's own span was already gone, and there was no undo.

That gap is closed, by porting `PartialWrite` and `KeystrokeSettle` from the C# core into `LexiconBarKit`. Three things are different now:

- **What goes out is always a prefix.** `UnicodeTyping.post` builds both key events for a chunk before it posts either, and stops at the first chunk it cannot build, so a write that falls short is the first n units of the replacement rather than the first n with a gap in them. Chunk boundaries also step back off a surrogate pair, so every point the write can stop at is a whole string. `typeUnicode` returns how many units went out.
- **What landed is read off the field, not off that count.** `KeystrokeSettle` polls the field across the same 0.6 s window the success path waits out, because posting only queues and the target app consumes on its own event loop. It matches what it finds against every "n of m units landed" state `PartialWrite` can describe, and keeps the count only as a bound on how much can have arrived, which is what lets it stop early instead of always burning the window. A single immediate read is how the first Windows version of this shipped green while still racing the keystrokes it was recovering from.
- **A half-written field is repaired, or else recorded.** A write known to have half-landed is carried down to the whole-value write, which is computed from the plan and so repairs the field whatever state it is in. Where that write cannot run or does not take, the undo ledger gets an entry for the state the field is actually in, so ⌃⌥Z puts your words back, and the message says "Only part of the correction went into `<app>`" instead of claiming nothing was written. Keystrokes that were posted and never appeared are deliberately **not** written over, because they would land on top of the value write: the engine stops, records what the field will hold if they do turn up, and says so, so a late arrival is still one hotkey from being undone.

The arithmetic and the settle are unit-tested in `LexiconBarKit`. That a short run of `CGEvent`s really leaves the field in one of the states the arithmetic describes is not, and cannot be tested here. See [What is not verified](#what-is-not-verified).
...
The decisions above are unit-tested. The Accessibility calls that carry them out are not: nothing in this repo can drive a real focused element in another app, so no test reaches a line that reads or writes one. This is the list rather than a vague disclaimer.

| Claim | Where | Why it is unproven |
| --- | --- | --- |
| The write ladder as a whole | `FixEverywhere/FixEngine.swift` `write` | The three strategies, their ordering, and every verification window in them (0.2 s, 0.6 s, 0.3 s). None of it has been executed; it has been read. |
| That a short keystroke run leaves a *prefix* in the field | `FixEngine.typeUnicode` | `UnicodeTyping.post` guarantees that a prefix is what was **posted**. That the app then consumed a prefix of that, rather than something else, is the premise `PartialWrite` reasons from. A field holding anything else settles as "unexplained" and stops the ladder, which is the safe answer, but that guard is itself unrun. |
| That the posted count is an exact upper bound | `FixEngine.typeUnicode` | `CGEvent.post` returns nothing, so an event dropped after it is handed to the window server is invisible here. It reads as "posted but never appeared", which stops the ladder and leaves the correction unmade rather than corrupting anything. Safe by construction, unverified in fact. |
| The 0.6 s settle window | `FixEngine.AXSettlePoll` | An app slower than that also reads as "posted but never appeared". Nothing is written and the field is as the user left it, so it is safe, but it makes the correction silently not happen, and the right number for a real app is unknown. |
| That the whole-value write repairs a half-written field | `FixEngine.write` | `AXValue` replaces the whole value, so in principle it does not care what state the field is in. Whether a given app's setter accepts a write while a selection is live is unrun. |
| That a recorded partial undo matches the real field | `FixEngine.recordPartial` | It re-reads the field and records nothing unless the read matches the arithmetic exactly. Both branches are unrun. |
| The live Accessibility path generally | `FixEverywhere/AXSupport.swift`, `FocusWatcher.swift` | Reading and writing `AXValue`, `AXSelectedText` and `AXSelectedTextRange`, the observers, and the caret rectangle. Verified by hand in TextEdit, the Codex desktop app and Safari; not verified by anything that runs in CI. |
