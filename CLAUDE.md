...
- `src/cli/` commander CLI (bin `lexicon`, 26 commands): `index.ts` only wires commander; handlers in `commands.ts` and `cmd-import.ts`, `cmd-install.ts`, `cmd-trust.ts`, `cmd-learn.ts`, `cmd-review.ts` (interactive harvest/review/edit), `cmd-serve.ts` (local HTTP API), `cmd-voice.ts` (whisper.cpp push-to-talk), `cmd-setup.ts` (onboarding wizard), `cmd-suggest.ts` (suggestions from history and hits); `prompt.ts` is the readline prompter.
